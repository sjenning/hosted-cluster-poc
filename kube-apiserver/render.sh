...
if [[ "$DEPLOY_HA" == "true" ]]; then
  envsubst < kube-apiserver-deployment-ha.yaml > ../manifests/managed/kube-apiserver-deployment.yaml
else
  envsubst < kube-apiserver-deployment.yaml.yaml > ../manifests/managed/kube-apiserver-deployment.yaml
fi
