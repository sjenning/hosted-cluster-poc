...
export DOCKER_BUILDER_IMAGE=$(podman run -ti --rm ${RELEASE_IMAGE} image docker-builder)
export DEPLOYER_IMAGE=$(podman run -ti --rm ${RELEASE_IMAGE} image deployer)
...
export OPENSHIFT_CONTROLLER_MANAGER_IMAGE=$(podman run -ti --rm ${RELEASE_IMAGE} image openshift-controller-manager)
